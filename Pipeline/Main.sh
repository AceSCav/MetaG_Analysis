...
echo "Note: A folder named <Results> will be created here with all the files provenient from this pipeline"
...
then
  ...
  Rscript decontam.R "$DATADIR" "$RESULTS_DIR"
  table_path="$RESULTS_DIR/table-decontam.qza"
else
  echo "Got it! Skipping contaminant filtering."
  table_path="$RESULTS_DIR/table-filtered-final.qza"
fi
